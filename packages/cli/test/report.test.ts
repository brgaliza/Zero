...
afterEach(async () => {
  await Promise.all(homes.splice(0).map((directory) => rm(directory, { force: true, recursive: true })));
  vi.restoreAllMocks();
});
...
describe("zero report", () => {
  it("persiste somente o schema sanitizado com permissões privadas", async () => {
    ...
    await expect(runReport({ homeDirectory: home, zeroVersion: "1.2.3", checks: unsafeChecks })).resolves.toMatchObject({ ok: true, code: "REPORT_CREATED" });
    ...
    expect(report).toMatchObject({ schemaVersion: 1, zeroVersion: "1.2.3", checks: [{ id: "docker", state: "desktop-stopped" }] });
